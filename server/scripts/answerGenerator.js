...
async function generateAnswer(question, articleUrl) {
  try {
    ...
    const { allSegments, topSimilarQuestions, topSimilarConversations } =
      await aiService.getRelevantContext(question);
...
async function main() {
  try {
    ...
    for (const article of articlesWithQuestions) {
      for (const articleQuestion of article.questions) {
        try {
          ...
          console.log(
            `Processed question ${totalQuestionsProcessed}: ${question}`
          );
        } catch (error) {
          console.error(
            `Failed to process question: ${articleQuestion}`,
            error
          );
        }
      }
    ...
    const outputPath = path.join(
      ...
      "..",
      "data",
      `answered_questions_${timestamp}.json`
    );
    await fs.writeFile(
      outputPath,
