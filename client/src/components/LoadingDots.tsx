...
const StyledDot = styled.div<StyledDotProps>`
  ...
  transform: translate(
    calc(${props => props.$x}px + ${props => props.$isAnimating ? '0px' : '0px'}), 
    ${props => props.$y}px
  );
  transition: transform 0.1s linear;
  
  ${({ $isAnimating, $animationDelay }) => $isAnimating && css`
    position: relative;
    transform: none;
...
const LoadingDots: React.FC = () => {
  const [positionHistory, setPositionHistory] = useState<PositionHistory[]>(() => {
    const initialPositions = Array(NUM_DOTS).fill(null).map((_, index) => ({
      x: index * GRID_SIZE,
      y: 0
    }));
  ...
  const updatePositions = useCallback(() => {
    ...
    setPositionHistory(prev => {
      ...
      const newHistory = [...prev.slice(-(NUM_DOTS + 1)), { positions: newPositions, direction: directionRef.current }];
      return newHistory;
    });
  }, [isStarted]);
  ...
  const currentPositions = positionHistory[positionHistory.length - 1].positions;
